@extends('layouts.app', ['activePage' => 'Áreas', 'titlePage' => __('Áreas')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            ...
                            <p class="card-category">Estas son las áreas registradas en el sistema</p>
                            <button type="button" class="btn btn-info" id="new" onclick="AddArea();">Agregar Área <i class="material-icons">add_circle_outline</i></button>
                        </div>
                        <div class="card-body">
...
{{--                                                </button>--}}
    ...
    <script>
        ...
        function AddArea() {
            $('#myModal').load( '{{ url('/areas/new') }}',function(response, status, xhr)
            {
                if (status == "success")
                    $('#myModal').modal('show');
            });
        ...
        function edit(button){
            ...
            $('#myModal').load( '{{ url('/areas/edit') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
        ...
        function deleted(button){
            console.warn('delete');
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/areas/delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
