@extends('layouts.app', ['activePage' => 'Fases', 'titlePage' => __('Fases')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title ">Lista de Fases</h4>
                            <p class="card-category">Estas son las fases registradas en el sistema</p>
                            <button type="button" class="btn btn-info" id="new" onclick="AddFase();">Agregar Fase <i class="material-icons">add_circle_outline</i></button>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered data-table">
                                    ...
                                    <th data-toggle="tooltip" data-placement="top" title="presiona para ordenar" style="cursor: pointer">Orden</th>
...
{{--                                                </button>--}}
    ...
    <script>
        ...
        function AddFase() {
            $('#myModal').load( '{{ url('/fases/new') }}',function(response, status, xhr)
            {
                if (status == "success")
                    $('#myModal').modal('show');
            });
        ...
        function edit(button){
            ...
            $('#myModal').load( '{{ url('/fases/edit') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
        ...
        function deleted(button){
            console.warn('delete');
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/fases/delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
