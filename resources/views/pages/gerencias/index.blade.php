@extends('layouts.app', ['activePage' => 'Gerencias', 'titlePage' => __('Gerencia')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title ">Lista de Gerencia</h4>
                            <p class="card-category">Estas son las gerencias registradas en el sistema</p>
                            <button type="button" class="btn btn-info" id="new" onclick="AddGerencia();">Agregar Gerencia <i class="material-icons">add_circle_outline</i></button>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered data-table">
                                    ...
                                    <th data-toggle="tooltip" data-placement="top" title="presiona para ordenar" style="cursor: pointer">Gerente</th>
                                    <th data-toggle="tooltip" data-placement="top" title="presiona para ordenar" style="cursor: pointer">Dirección</th>
...
{{--                                                </button>--}}
    ...
    <script>
        ...
        function AddGerencia() {
            $('#myModal').load( '{{ url('/gerencias/new') }}',function(response, status, xhr)
            {
                if (status == "success")
                    $('#myModal').modal('show');
            });
        ...
        function edit(button){
            ...
            $('#myModal').load( '{{ url('/gerencias/edit') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
        ...
        function deleted(button){
            console.warn('delete');
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/gerencias/delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
