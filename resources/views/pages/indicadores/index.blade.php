@extends('layouts.app', ['activePage' => 'Indicadores', 'titlePage' => __('Indicadores')])
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            ...
                            <p class="card-category">Estos son los indicadores registrados en el sistema</p>
                            <button type="button" class="btn btn-info" id="new" onclick="AddIndicador();">Agregar Indicador <i class="material-icons">add_circle_outline</i></button>
                        </div>
                        <div class="card-body">
...
{{--                                                </button>--}}
    ...
    <script>
        ...
        function AddIndicador() {
            $('#myModal').load( '{{ url('/indicadores/new') }}',function(response, status, xhr)
            {
                if (status == "success")
                    $('#myModal').modal('show');
            });
        ...
        function edit(button){
            ...
            $('#myModal').load( '{{ url('/indicadores/edit') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
        ...
        function deleted(button){
            console.warn('delete');
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/indicadores/delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
