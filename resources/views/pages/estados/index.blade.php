                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title ">Tipos de Estado</h4>
                            <p class="card-category">Estos son los distintos estados registrados para el sistema</p>
                            <button type="button" class="btn btn-info" id="new" onclick="AddEstado();">Agregar Estado <i class="material-icons">add_circle_outline</i></button>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered data-table">
                                    ...
                                    <th data-toggle="tooltip" data-placement="top" title="presiona para ordenar" style="cursor: pointer">Activo</th>
...
{{--                                                </button>--}}
    ...
    <script>
        ...
        function AddEstado() {
            $('#myModal').load( '{{ url('/estados/new') }}',function(response, status, xhr)
            {
                if (status == "success")
                    $('#myModal').modal('show');
            });
        ...
        function edit(button){
            ...
            $('#myModal').load( '{{ url('/estados/edit') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
        ...
        function deleted(button){
            console.warn('delete');
            var clave = $(button).attr('clave');
            $('#myModal').load( '{{ url('/estados/delete') }}/'+clave,function(response, status, xhr){
                if ( status == "success" ) {
                    $('#myModal').modal('show');
                }
            } );
